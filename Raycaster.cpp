...
namespace sfray {
    ...
    void Raycaster::drawForCamera(sf::RenderWindow &window, sfray::Camera &camera, sf::View& view){
        ...
        for (unsigned int x = 0; x < mGfxWidth; ++x){
            ...
            float floorX;
            if (side == 1) {
                floorX = rayPosX + ((mapY - rayPosY + (1 - stepY) / 2) / rayDirY) * rayDirX;
            }
            else {
                floorX = rayPosY + ((mapX - rayPosX + (1 - stepX) / 2) / rayDirX) * rayDirY;
            }
            floorX -= floor(floorX);
            // 4 different wall directions possible
            if (side == 0 && rayDirX > 0){
                floorXWall = mapX;
                floorYWall = mapY + floorX;
            }
            ...
            {
                for (unsigned int y = mGfxHeight / 2; y < mGfxHeight; ++y){
                    ...
                    if (int(currentFloorX) >= mMap.getWidth() || int(currentFloorY) >= mMap.getHeight()
                        || int(currentFloorX) < 0 || int(currentFloorY) < 0){
                        continue;
                    }
